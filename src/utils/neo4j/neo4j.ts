...
export const getData = async () => {
  const query = `
MATCH (n)
WHERE n:backendFramework OR n:Database OR n:frontendFramework OR n:Language OR n:metaFramework OR n:Service OR n:cssFramework
...
export const getTechsForFeature = async (featureName: string | string[]) => {
 ...
 MATCH (t)-[r:SUPPORTS]->(f:Feature)
  ...
  AND any(label IN labels(t) WHERE label IN ['frontendFramework', 'backendFramework','cssFramework' ,'Database', 'Language', 'metaFramework', 'Service'])
